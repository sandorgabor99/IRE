...
public class AccountView implements Serializable {
    ...
    public void transferMoney() {
        ...
        if(sender.getBalance()-money_out >0){
    ...
    public ArrayList<Users> getActiveUsersWithoutAccount(){
        ...
        ArrayList<Users> activeUsersWithout = new ArrayList<>();
        if(accounts.isEmpty()){
            for(Users u : userService.getAllUsers()){
                activeUsersWithout.add(u);
            }
        }
        else {
            for (Account a : accounts) {
                System.out.println("Checking : \n " + a.toString());
                if (service.checkIfUserHasNotAccount(a.getUser().getId()) == true && a.getUser().isActiveUser() == true) {
                    System.out.println(a.toString());
                    activeUsersWithout.add(a.getUser());
                }
            }
        }
        return activeUsersWithout;
    }
